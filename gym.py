...
class Instructor:
    ...
    def can_teach(self, workout_class: WorkoutClass) -> bool:
        ...
        True
        """
        return set(workout_class.get_required_certificates()).issubset\
            (set(self._certificates))
...
class Gym:
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        if time_point in self._schedule:
            if room_name in self._schedule[time_point]:
                return False
            for room_loop in self._schedule[time_point]:
                if instr_id == self._schedule[time_point][room_loop][0].\
                get_id():
                    return False
            if self._instructors[instr_id].can_teach\
            (self._workouts[workout_name]) is False:
                return False
            else:
                self._schedule[time_point][room_name] = (self._instructors\
                [instr_id], self._workouts[workout_name], [])
        if time_point not in self._schedule:
            if self._instructors[instr_id].can_teach(self._workouts\
            [workout_name]) is True:
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        for room_name in self._schedule[time_point]:
            if client in self._schedule[time_point][room_name][2]:
                return False
        for room_name in self._schedule[time_point]:
            if workout_name == self._schedule[time_point][room_name][1]\
            .get_name():
    ...
    def offerings_at(self, time_point: datetime) -> List[Tuple[str, str, str]]:
        ...
        if time_point in self._schedule:
            for room_name in self._schedule[time_point]:
                element = (self._schedule[time_point][room_name][0].name,\
                self._schedule[time_point][room_name][1].get_name(), room_name)
                offerings.append(element)
        if time_point not in self._schedule:
            return []
        return offerings
    ...
    def instructor_hours(self, time1: datetime, time2: datetime) -> \
        ...
        for instr_id in self._instructors:
            work_hours[instr_id] = 0
        for time_point in self._schedule:
            if time1 <= time_point <= time2:
                for room_name in self._schedule[time_point]:
                    instr_id = self._schedule[time_point][room_name][0].get_id()
                    work_hours[instr_id] += 1
        return work_hours
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        for instructor in self._instructors:
            instructor = self._instructors[instructor]
            hours_worked = self.instructor_hours(time1, time2)\
            [instructor.get_id()]
            salary = float((base_rate*hours_worked) + \
            (hours_worked*BONUS_RATE*instructor.get_num_certificates()))
            pay_tuple = (instructor.get_id(), instructor.name, hours_worked,\
            salary)
            pay_list.append(pay_tuple)
            pay_list.sort()
        return pay_list
